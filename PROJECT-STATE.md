# 概念学习树 — 项目状态总览（2026-09-01 更新：第十八轮 打包脚手架 + 上手引导 + 数据安全 + 安全审计）

## 当前状态

- **第十八轮完成**：① 打包脚手架（Electron 服务改**进程内运行**、打包态数据进 `AppData`、`electron-builder.yml` + `npm run dist` + 图标；沙箱因缺符号链接权限无法出包，**需本机跑 `npm run dist`**）；② 首次进入**欢迎引导** `WelcomeModal`（一次性 + 一键载入「机器学习」示例树）；③ 数据安全**损坏文件隔离**（坏 JSON 先留存不覆盖）；④ 安全审计：修复备份恢复**路径穿越**、删除无用 `react-router-dom`（生产依赖**0 漏洞**）、静态服务/请求体/DPAPI/Electron/XSS 复核通过（记录 §33）
- **第十七轮完成**：把 `--ct-glow-primary` 设为 `transparent`，去掉所有按钮/控件的紫色光晕；删除画布圆形按钮 `.ct-orb::before` 的悬停扫光；主页 logo 光晕单独保留（记录 §32）
- **第十五/十六轮**：移除 `BorderGlow` 边缘辉光恢复朴素边框（§30）；修复 `FancySelect` 下拉向下弹被裁 → 自动向上弹（§31）
- **仓库已公开化准备**：单一干净提交、作者为 GitHub noreply 邮箱、无个人数据/密钥；README 已含下载/安装/隐私说明；全新克隆实测可 install→build→run
- 门禁全绿：**单测 243/243** + **e2e 7/7**、**tsc 零错误**、build 成功
- 备份标签：`backup/pre-round18`、`backup/pre-glow-removal`、`backup/pre-fs-flip`、`backup/pre-remove-glow`、`backup/pre-optimize-2`、`backup/pre-optimize`、`backup/pre-entrance-anim`、`backup/pre-component-ui`、`backup/pre-ui-overhaul`
