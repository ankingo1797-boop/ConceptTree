...
interface SeriesPageProps {
  ...
  introActive?: boolean
  /** 第十八轮：欢迎引导里「配置 AI」跳设置（由 App 打开设置页） */
  onOpenSettings?: () => void
}

export default function SeriesPage({ model, openId = null, onOpenId, onSeriesOpen, calendarRequest, statsRequest, locateRequest, introActive = false, onOpenSettings }: SeriesPageProps) {
  const [data, setData] = React.useState<ConceptTreeData | null>(null)
  const [error, setError] = React.useState<string | null>(null)
  const [name, setName] = React.useState('')
  // 第十八轮：首次进入的欢迎引导（localStorage 记一次，之后不再弹）
  const [welcomed, setWelcomed] = React.useState(() => { try { return localStorage.getItem('ct-welcomed') === '1' } catch { return false } })
  const markWelcomed = () => { setWelcomed(true); try { localStorage.setItem('ct-welcomed', '1') } catch { /* 忽略 */ } }
  ...
  // 第十八轮：一键载入一棵「机器学习」示例树，让新装用户立刻看到成品效果
  const loadSample = () => {
    const ts = new Date().toISOString()
    const concepts: Record<string, Concept> = {}
    const edges: { id: string; from: string; to: string; type: 'parent-child' }[] = []
    const add = (id: string, pname: string, parentId: string | null, status: Concept['status'], summary: string) => {
      concepts[id] = { id, name: pname, summary, parentId, sessionId: null, status, x: null, y: null, notes: '', history: [], candidates: [], createdAt: ts, updatedAt: ts }
      if (parentId) edges.push({ id: uid('e'), from: parentId, to: id, type: 'parent-child' })
    }
    const rid = uid('c')
    add(rid, '机器学习', null, 'learning', '研究如何用数据让计算机自动改进性能')
    add(uid('c'), '监督学习', rid, 'learned', '用带标签样本学习输入→输出的映射（分类、回归）')
    add(uid('c'), '无监督学习', rid, 'unlearned', '从无标签数据中发现结构（聚类、降维）')
    const dl = uid('c')
    add(dl, '深度学习', rid, 'learning', '用多层神经网络学习层次化表示')
    add(uid('c'), '神经网络', dl, 'unlearned', '由多层神经元构成的可微模型')
    add(uid('c'), 'Transformer', dl, 'doubtful', '用自注意力机制建模序列的架构')
    const sid = uid('s')
    const series: Series = { id: sid, name: '机器学习（示例）', rootConceptId: rid, createdAt: ts, updatedAt: ts, concepts, edges }
    const next = { ...data, series: { ...data.series, [sid]: series } }
    persist(next).then(() => { markWelcomed(); setOpenId(sid) })
  }
  ...
  return (
    <div style={styles.page}>
      ...
      {/* 第十八轮：首次进入的欢迎引导（一次性，localStorage 记状态） */}
      {!welcomed && (
        <WelcomeModal
          onConfigure={() => { markWelcomed(); onOpenSettings && onOpenSettings() }}
          onLoadSample={loadSample}
          onStart={markWelcomed}
        />
      )}
