...
export default function FancySelect({ value, options, onChange, ariaLabel, title }: FancySelectProps) {
  ...
  const [dropUp, setDropUp] = React.useState(false) // 下方空间不足时向上弹
  ...
  const toggle = () => {
    ...
    if (next && wrapRef.current) {
      ...
      const estListH = Math.min(options.length, 6) * 34 + 12
      setDropUp(spaceBelow < estListH + 8)
    }
    setOpen(next)
  ...
  return (
    <div className="ct-fs" ref={wrapRef}>
      ...
      {open && (
        <div className={`ct-fs-list${dropUp ? ' ct-fs-list--up' : ''}`} role="listbox" aria-label={ariaLabel}>
          {options.map((o, i) => (
            <button
              key={o.value}
              type="button"
