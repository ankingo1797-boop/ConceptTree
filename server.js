...
// 读取学习数据；若文件存在但损坏，先隔离留存（备份目录 + 原文件改名）再回落空结构，绝不静默覆盖丢数据
function readDataJson() {
  const fallback = { version: 1, series: {} }
  if (!existsSync(DATA_FILE)) return fallback
  try {
    ...
    try {
      ...
      copyFileSync(DATA_FILE, join(BACKUP_DIR, 'concept-tree-corrupt-' + stamp + '.json'))
    } catch (e) { logError('损坏数据隔离备份失败: ' + e.message) }
    renameSyncSafe(DATA_FILE, DATA_FILE + '.corrupt-' + stamp)
    logWarn('数据文件损坏，已隔离留存并以空数据启动（' + stamp + '）')
    return fallback
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/data' && method === 'GET') {
      ...
      return json(res, 200, readDataJson())
    }
    ...
    if (restoreMatch && method === 'POST') {
      ...
      // 安全：只接受合法备份文件名，拒绝任何路径分隔符 / .. （防编码后的 ../ 穿越）
      if (!/^concept-tree-.+\.json$/.test(file) || file.includes('/') || file.includes('\\') || file.includes('..')) {
...
// 启动服务（导出供 Electron 主进程进程内调用；`node server.js` 直接执行时自动启动）
...
// 被 `node server.js` 直接执行 → 自动启动；被 Electron 主进程 import → 由调用方设 CT_NO_AUTOSTART=1 并显式 startServer()
if (process.env.CT_NO_AUTOSTART !== '1') startServer()
